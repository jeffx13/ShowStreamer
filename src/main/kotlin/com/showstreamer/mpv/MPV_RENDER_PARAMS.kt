...
class mpv_render_param(): Structure() {
    ...
    override fun getFieldOrder(): MutableList<String> {
        return mutableListOf("type", "data")
    }
    fun setValues(type:Int,data:Pointer?){
        this.type=type
        this.data=data
    }
}

@FieldOrder("get_proc_address", "get_proc_address_ctx", "extra_exts")
class mpv_opengl_init_params : Structure() {
    @JvmField
    var get_proc_address: get_proc_address? = null
    @JvmField
...
interface get_proc_address : Callback {
    fun callback(handle: Pointer?, name: String?): Pointer?
}




enum class mpv_render_param_type(val id:Int) {
    /**
