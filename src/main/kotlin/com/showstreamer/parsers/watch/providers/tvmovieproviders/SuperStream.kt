...
import com.showstreamer.parsers.watch.providers.tvmovieproviders.SuperStream.CipherUtils.getVerify
...
class SuperStream : ShowProvider() {
    ...
    override suspend fun search(query: String, page: Int, type: TvType): List<ShowResponse> {
        ...
        println(queryApi(apiQuery).text)
        val searchResponse = (queryApi(apiQuery).text).parsed<MainData>().data.mapNotNull {
            val type = if (it.boxType == 1) TvType.Movie else TvType.TvSeries
            println(it)
            //println(LoadData(it.id ?: return@mapNotNull null, it.boxType))
            //ShowResponse(it.title,it.)
//            ShowResponse(it.title!!,
...
//        val loadData = show.url.parsed<LoadData>() //parseJson<LoadData>(url)
...
//        }
    ...
    private object CipherUtils {
        ...
        fun encrypt(str: String, key: String, iv: String): String? {
            return try {
                ...
                //Base64.getEncoder().encodeToString(cipher.doFinal(str.toByteArray()))
                cipher.doFinal(str.toByteArray()).base64Encoded()
            } catch (e: Exception) {
                e.printStackTrace()
                null
            }
        ...
        fun md5(str: String)= MD5Util.md5(str)?.let { HexDump.toHexString(it).lowercase() }
    ...
    private suspend fun queryApi(query: String): NiceResponse {
        ...
        val base64Body = newBody.base64Encoded()//String(Base64.encode(newBody.toByteArray(), Base64.DEFAULT))
        ...
        println(Jsoup.connect(apiUrl).headers(headers).data(data).post())
    ...
    private fun getExpiryDate(): Long = unixTimeMillis + 60 * 60 * 12
    ...
    private val iv = "d0VpcGhUbiE=".base64Decoded()
    private val key = "MTIzZDZjZWRmNjI2ZHk1NDIzM2FhMXc2".base64Decoded()
    private val ip = "aHR0cHM6Ly8xNTIuMzIuMTQ5LjE2MA==".base64Decoded()
    private val apiUrl =
        "$ip${"L2FwaS9hcGlfY2xpZW50L2luZGV4Lw==".base64Decoded()}"
    private val appKey = "bW92aWVib3g=".base64Decoded()
    private val appId = "Y29tLnRkby5zaG93Ym94".base64Decoded()
